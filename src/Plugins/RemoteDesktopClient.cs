...
{
    /// <summary>
    /// Taken from http://stackoverflow.com/a/220870/42926
    /// </summary>
    ...
    public class RemoteDesktopClient : BaseActOnTypedItem<FileInfo>, ICanActOnTypedItem<FileInfo>,
                                       ...
                                       IActOnTypedItemWithAutoCompletedArguments<FileInfo>
    {
        ...
        {
            var sessionsKey = Registry.CurrentUser.OpenSubKey(@"\Software\Microsoft\Terminal Server Client\Default");
            if (sessionsKey == null) _sessionNames = new string[] {};
            else _sessionNames = 
                sessionsKey.GetValueNames()
                           .Select(name => (string)sessionsKey.GetValue(name, string.Empty)).ToArray();
        }
        ...
        {
            get { return "Launch remote desktop session"; }
        }

        public bool CanActOn(FileInfo item)
        {
            var path = item.FullName;
            if(item.Extension.ToLowerInvariant() == ".lnk")
                path = ShortcutHelper.ResolveShortcut(item.FullName);
            ...
            return fileName.Equals("mstsc.exe", StringComparison.InvariantCultureIgnoreCase);
        }
    }
}